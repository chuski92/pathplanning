...
def goal_in_ob(goal, ob):

    for i in range(len(ob)):
        if(goal[0]==ob[i][0] and goal[1]==ob[i][1]):
            return True
    return False
...
def main():
    ...
    # start and goal position dijkstra
    sx = 1.0  # [m]
    sy = 1.0  # [m]
    gx = 8.0  # [m]
    gy = 8.0  # [m]
    ...
    for j in range(end):
        for i in range(1000):
            goal = np.array([rx[j], ry[j]])
            print goal
            print goal_in_ob(goal,ob)
            if goal_in_ob(goal,ob):
                break
